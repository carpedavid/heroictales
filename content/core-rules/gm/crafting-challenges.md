---
title: "Crafting Challenges"
date: 2020-10-28T20:05:49-04:00
draft: false
tags: [gm's companion]
description: "As a GM, you get the opportunity to craft the challenges that the players will attempt to overcome"
summary: "As a GM, you get the opportunity to craft the challenges that the players will attempt to overcome. A challenge can be anything from fighting a colossal, tentacled, city-devouring monster to developing the antidote for a magical zombie-creating poison."
weight: 8
---

As a GM, you get the opportunity to craft the *challenges* that the players will attempt to overcome. A challenge can be anything from fighting a colossal, tentacled, city-devouring monster to developing the antidote for a magical zombie-creating poison. It could involve persuading the Grand Duchess to sponsor your voyage to the Islands East of Forever or it might involve helping an old, friendly vampire rescue his beloved spider-cat, Whiskers, from the Haunted Mines of Terror and Dread.

As a result, creating an engaging challenge is more of an art than a science. It’s important to understand the expectations that each of the players brings to the group and to create a chance for each of them to contribute. Try to minimize creating challenges that highlight just one character’s abilities — they can be interesting on occasion, but can become tedious if overused.

To create a compelling challenge, you’ll want to think through the following items:

* **Objective** — what is the team trying to accomplish?
* **Elements** — where and when does the challenge take place?
* **Obstacles, Timeframe, and Constraints** — what’s standing in the way of success, and what could cause them to fail?
* **Milestones** — as the characters make progress toward their goal, how does the scene change?
* **Complications** — when a player suffers a setback, what does it look like?
* **Success and Failure** — what are the results of completing a challenge, both positive and negative?
* **Inertia** — how challenging is the challenge?

## Objective
A shared understanding of the primary objective of a challenge is crucial. What is the team trying to accomplish and why? This needs to be clearly communicated between the GM and players in order for the story to flow cleanly.

## Elements
What can you describe to the players so that the characters have a sense of place and purpose? What does the location of the challenge look like? Sound like? Smell like? Are the characters physically present with each other or are they communicating in some other fashion?

Are there people present? What are their names and personalities? Are there creatures? Monsters? Self-aware, super-intelligent math formulae?

If you’re comfortable coming up with the elements of the scene on the fly, don’t hesitate to do so. If you need time to prepare and think things through, then create a bullet-point list of things to remember.

## Obstacles, Timeframe, and Constraints
First, what’s going to make the challenge a challenge? Something should be standing in the team’s way (sometimes literally) in order for there to be a chance of failure. There may be multiple obstacles. Fighting the giant demon on a slender bridge above a pit of bubbling lava is much more cinematic than fighting it in the middle of a featureless cavern.

Second, how long does the team have to successfully complete the challenge? Tasks that are, if not trivial, then straightforward when not constrained can become an extreme test of teamwork when a tight deadline with serious consequences looms. For example, developing a magical potion that cures the crown princess’ petrified court might be important, but if the team can take their time, it will feel much less urgent if they can take their time than if they have to complete it by the next dawn.

Finally, what are the other boundaries of the challenge?  Does the team need to sneak into the rival gang-leader’s lair without being spotted? Do they need to navigate the minotaur’s maze without attracting its attention? Creative use of constraints can elevate a mundane challenge into one the players will remember.

## Milestones
As the scene unfolds, how does it change? What happens? An engaging challenge should be dynamic and react to the actions of the characters. The best practice is to have the scene meaningfully change every three rolls or so. This prevents the action from becoming stale, and it prevents the players from running out of interesting things to do.

Therefore, a hard challenge should have at least two milestones and a normal challenge one milestone. For an easy challenge, the milestone is optional. 

For example, perhaps the second wave of zombie ninja arrives, and this wave is armed with electrified tonfa. Perhaps the characters lop the skyscraper-sized tentacles off the great old one and now they have to prevent the twitching limbs from destroying the city while also closing the gate to the Far Realm.

## Complications
It can also be helpful to have a few *complications* handy for when a player rolls poorly. A good complication illustrates that the situation just got worse, but still allows the characters the freedom to continue working as a team.

For example, Kristivn “Kris” Kristvinson, the dwarven barbarian, rolls a complication while his team is battling skeletons near the entrance of the Höhle der Winde. You decide that the colossal cave system is filled with traps, so you describe Kris falling into a ten-foot-deep pit. Now Kris’ player has to narrate climbing out of the trap (or having his teammates help him) before he can get back to swinging his battle axe.

Mechanically, the team has taken a penalty for the bad roll by losing momentum. The complication is a narrative device that works in tandem with momentum to reflect the setback in the story.

## Success and Failure
The players will eventually reduce the *inertia* of the challenge to zero or they will lose all of their *momentum*. What happens in either scenario?

Success should mean that the characters achieve their objective in one form or another. If they are battling a great dragon, the dragon might surrender, it might die, or it might offer to trade its treasure horde for its life. If they are exploring the endlessly deep Höhle der Winde, success means making it through to the next waypoint without getting lost.

Failure, on the other hand, can mean many things but it should always propel the narrative forward; failure should never mean that the story comes to a halt and the party gets stuck. Perhaps the group realizes that the great dragon has the upper hand as it physically picks them up and tosses them out of its lair. Perhaps the spelunkers wander, lost in the crevasses of Höhle der Winde, and make it to their checkpoint three days late, missing a key contact by a few hours. Now they have to implement Plan B and formulate an alternate plan to infiltrate the Koboldkönig’s lair.

## Inertia
The inertia of a challenge can range from one to fifty. The appropriate value for a particular challenge depends on the team’s *power level*, the number of characters participating in the challenge, and how hard or easy you want the challenge to be.

Consult the tables below to determine the appropriate inertia for your challenge. These numbers are based on the standard size of a player’s dice pool at any given power level. If a character has advantage, or a player incorporates their *traits* into their actions, then the challenge will be easier. However, if a character is suffering from disadvantage, then the challenge will be harder.

You can vary the inertia of individual challenges in order to create variety, as long as you use the provided tables as a guide.

### One Player
| Power Level | Easy | Normal | Hard |
|-------------|------|--------|------|
| 1-2         | 0    | 2      | 2    |
| 3-4         | 2    | 2      | 4    |
| 5-6         | 2    | 2      | 4    |
| 7-8         | 2    | 4      | 6    |
| 9-10        | 2    | 4      | 6    |

### Two Players
| Power Level | Easy | Normal | Hard |
|-------------|------|--------|------|
| 1-2         | 0    | 2      | 2    |
| 3-4         | 2    | 4      | 6    |
| 5-6         | 2    | 6      | 8    |
| 7-8         | 4    | 8      | 10   |
| 9-10        | 4    | 8      | 12   |

### Three Players
| Power Level | Easy | Normal | Hard |
|-------------|------|--------|------|
| 1-2         | 2    | 2      | 4    |
| 3-4         | 4    | 6      | 10   |
| 5-6         | 4    | 8      | 10   |
| 7-8         | 6    | 12     | 16   |
| 9-10        | 6    | 12     | 18   |

### Four Players
| Power Level | Easy | Normal | Hard |
|-------------|------|--------|------|
| 1-2         | 2    | 2      | 4    |
| 3-4         | 4    | 8      | 12   |
| 5-6         | 6    | 10     | 14   |
| 7-8         | 8    | 14     | 22   |
| 9-10        | 8    | 18     | 26   |

### Five Players
| Power Level | Easy | Normal | Hard |
|-------------|------|--------|------|
| 1-2         | 2    | 4      | 4    |
| 3-4         | 6    | 10     | 14   |
| 5-6         | 6    | 12     | 18   |
| 7-8         | 10   | 18     | 28   |
| 9-10        | 10   | 20     | 32   |

### Six Players
| Power Level | Easy | Normal | Hard |
|-------------|------|--------|------|
| 1-2         | 2    | 4      | 6    |
| 3-4         | 6    | 12     | 18   |
| 5-6         | 8    | 14     | 20   |
| 7-8         | 12   | 22     | 32   |
| 9-10        | 14   | 26     | 38   |

## Adventures and Campaigns
Just as a novel is constructed from chapters, which themselves are constructed from a series of scenes, you can link challenges together to form a long, cohesive narrative.

A series of challenges linked by a plot is known as an *adventure*. An adventure is much like a chapter of a novel that is constructed from multiple scenes. Adventures vary in length, but at the short end they might comprise three to five challenges and at the long end of the spectrum ten to twelve. The end of an adventure is often a good time to increase the team’s power level.

A *campaign* is a set of adventures linked by a a common plot (or at least a common set of characters). Over the course of a campaign, the team grows, learns, and changes. A campaign often has an overarching goal that individual adventures may directly address. For example, in the Journey to the Fire Crane campaign, a team of young warriors took their clan’s offering  to the far reaches of the empire. Along the way, they fought rival clans, brokered peace between warring factions of arachnids, and took a young, abandoned girl under their wing. Ultimately, the team delivered their offering to the guardian spirit known as the Fire Crane and secured good fortune for their clan.